package com.sh.hairball.webchat.controller;

import java.io.IOException;
import java.sql.Connection;

import javax.websocket.OnClose;
import javax.websocket.OnMessage;
import javax.websocket.OnOpen;
import javax.websocket.Session;
import javax.websocket.server.ServerEndpoint;

import com.sh.hairball.webchat.model.WebChat;
//관리자 유저에서 서버간의 웹 소켓 url
@ServerEndpoint("/admin")
public class Admin {
	// 관리자 유저는 하나라고 가정하고 만약 둘 이상의 세션에서 접속을 하면 마지막 세션만 작동한다.
	private static Session admin = null;
	// 관리자 유저가 접속을 하면 발생하는 이벤트 함수
	@OnOpen
	public void handleOpen(Session userSession) {
		// 기존에 관리자 유저의 소켓이 접속중이라면
		if (admin != null) {
			try {
				// 접속을 끊는다.
				admin.close();
			} catch (IOException e) {

			}
		}
		// 새로 접속한 관리자 클라이언트의 세션을 저장
		admin = userSession;
		// 기존에 접속해 있는 유저의 정보를 관리자 client로 보낸다.
		for(String key : BroadSocket.getUserKeys()) {
			// 전송
			visit(key);
		}
	}
	// 관리자 유저가 메시지를 보내면 발생하는 이벤트
	@OnMessage
	public void handleMessage(String message, Session userSession) throws IOException {
		// 메시지 형식은 "key#####message"로 가정
		String[] split = message.split("#####", 2);
		// 앞은 유저의 key 데이터
		String key = split[0];
		// 뒤 정보는 메시지
		String msg = split[1];
		// 해당 키를 가지는 일반 사용자에게 메시지를 전달
		BroadSocket.sendMessage(key, msg);
		// 사용자의 대화는 키로 고유하게 식별되는 별도의 영역에서 처리된다
	}
	// 접속이 끊기면 위 관리자 세션을 null 처리한다.
	@OnClose
	public void handleClose(Session userSession) {
		// 관리자 세션을 null로 초기화하여 관리자가 접속을 종료했음을 표시
		admin = null;
	}
	// 관리자 유저로 메시지를 보내는 함수
	private static void send(String message) {
		if (admin != null) {
			try {
				admin.getBasicRemote().sendText(message);
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
	}
	// 일반 유저가 접속했을 때, 관리자 유저에게 알리는 함수
	public static void visit(String key) {
		// json 구조로 status는 visit이고 key는 유저 키 정보이다.(javascript와 맞추는 프로토콜)
		send("{\"status\":\"visit\", \"key\":\"" + key + "\"}");
	}
	// 일반 유저가 메시지를 보낼 때, 관리자 유저에게 알리는 함수
	public static void sendMessage(String key, String message) {
		// json 구조로 status는 message이고 key는 유저 키 정보이다.(javascript와 맞추는 프로토콜) message는 보내는 메시지이다.
		send("{\"status\":\"message\", \"key\":\"" + key + "\", \"message\":\"" + message + "\"}");
	}
	// 일반 유저가 접속을 끊을 때, 관리자 유저에게 알리는 함수
	public static void bye(String key) {
		// json 구조로 status는 bye이고 key는 유저 키 정보이다.(javascript와 맞추는 프로토콜)
		send("{\"status\":\"bye\", \"key\":\"" + key + "\"}");
	}
}